...
describe('Orchestrator (e2e with Testcontainers)', () => {
    ...
    beforeEach(async () => {
        ...
        const appContainer =
            process.env.E2E_BUILD_IMAGE === 'true'
                ? await GenericContainer.fromDockerfile(path.resolve(__dirname, '..')).withTarget('test-app').build()
    ...
    it('POST /api/v1/transactions creates a transaction and GET retrieves it', async () => {
        ...
        expect({ status: createResponse.status, body: createBodyText }).toMatchObject({
            status: 201,
        });
